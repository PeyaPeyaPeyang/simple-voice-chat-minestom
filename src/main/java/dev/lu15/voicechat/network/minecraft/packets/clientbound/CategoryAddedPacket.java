...
public record CategoryAddedPacket(@NotNull String identifier, @NotNull Category category)
        implements Packet<CategoryAddedPacket> {
    ...
    public CategoryAddedPacket {
        if (!IDENTIFIER_PATTERN.matcher(identifier).matches())
            throw new IllegalArgumentException("category id does not match pattern " + IDENTIFIER_PATTERN.pattern());
        if (category.name().length() > 16)
            throw new IllegalArgumentException("category name is too long, found " + category.name()
                    .length() + " characters, maximum is 16");
        if (category.description() != null && category.description().length() > 32767)
            throw new IllegalArgumentException("category description is too long, found " + category.description()
                    .length() + " characters, maximum is 32767");
        if (category.icon() != null) {
            if (category.icon().length != 16)
                throw new IllegalArgumentException("category icon is not 16x16, found " + category.icon().length + "x" + category.icon().length);
            for (int[] row : category.icon()) {
                if (row.length != 16)
                    throw new IllegalArgumentException("category icon is not 16x16, found " + row.length + "x16");
            }
        }
    }
